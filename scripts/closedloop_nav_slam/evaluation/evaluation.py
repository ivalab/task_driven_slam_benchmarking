...
class Evaluation:
    ...
    def __find_start_timestamp(self, planned_wpts, reached_stamped_wpts) -> float:
        ...
        if reached_stamped_wpts.shape[0] <= index + 1:
            ...
            timestamp = reached_stamped_wpts[-1, 0] + 1e-3 # safe margin, ignore all mapping data
        else:
            timestamp = reached_stamped_wpts[index, 0] + 1e-3 # safe margin
        return timestamp
